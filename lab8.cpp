...
//   --------------------------------------------
//   bodySurfaceArea * verticalTrunkCircumference#include <stdio.h>
#include <iostream>
#include <cstdlib>
#include <ctime>
#include <cctype>
...
int unit(float tolerance) {
    SBSI sbsi;
    int bad = 0;
    cout << "H    WC   BSA    VTC  SBSI      Function\n";
    cout << "---  ---  -----  ---  --------  ----------\n";
    for (int i = 0; i < 27*3; i++) {
        sbsi.h = values[i * 5 + 0];
        sbsi.wc = values[i * 5 + 1];
        sbsi.bsa = values[i * 5 + 3];
        sbsi.vtc = values[i * 5 + 2];
        Flt val = bodyShapeIndex(&sbsi);
        printf("%3.0f %4.0f %6.0f %4.0f %10.7f %10.7f \n", 
                sbsi.h, sbsi.wc, sbsi.bsa, sbsi.vtc, values[i*5+4], val);
        if (abs(values[i*5+4] - val) >= tolerance) 
            bad++;
    }
    return bad;
}
